...
public class TicTacToeClient {
   ...
   //If the answer is no, the loop is exited and the server is sent a "QUIT" message.  If an OPPONENT_QUIT message is recevied then the loop will exit and the server will be sent a "QUIT" message also.
    public void play() throws Exception {
        ...
        try {
            ...
            if (response.startsWith("WELCOME")) {
                ...
                icon = new ImageIcon(mark == 'X' ? getClass().getClassLoader().getResource("xx.gif") : getClass().getClassLoader().getResource("oo.gif"));
                opponentIcon  = new ImageIcon(mark == 'X' ? getClass().getClassLoader().getResource("oo.gif") : getClass().getClassLoader().getResource("xx.gif"));
    ...
    static class Square extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -4850724832158121245L;
    ...
    public static void main(String[] args) throws Exception {
        while (true) {
            ...
            TicTacToeClient client = new TicTacToeClient("192.168.0.3");// serverAddress);
